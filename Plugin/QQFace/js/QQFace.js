require("../css/QQFace.css");
/*
 @ 文本框插入表情插件
*/
var faceImgArr = [];
for(var i = 0; i < 60; i++){  //循环创建60个表情
	faceImgArr.push(require("../../../Plugin/QQFace/images/"+(i+1)+".gif"));
}
...
$.fn.smohanfacebox = function(options) {
	...
	//创建表情框
	var faceimg = '';
	faceImgArr.map(function(item,index){
		faceimg+='<li><a href="javascript:void(0)"><img src="'+item+'" face="<emt>'+(index+1)+'</emt>"/></a></li>';
	});
	...
	$facepic
		...
		.on('click',function(){
			 ...
			 $("#"+options.textid)
				 .off('click',function(){})
				 .insertContent($(this).attr("face"),0,options.callback);
			 $btn.removeClass('in');
		});
	//关闭表情层
...
// 【漫画】 光标定位插件
$.fn.extend({
	insertContent : function(myValue, t, callback) {
		var $t = $(this)[0];
...
$.fn.extend({
	replaceFace: function(faces){
		for(i=0;i<60;i++){
			faces = faces.replace(eval("/<emt>"+(i+1)+"<\\/emt>/g"),'<img data-id="'+(i+1)+'" src="'+faceImgArr[i]+'"/>');
		}
		$(this).html(faces);
		return faces;
	},
	replaceText: function(text,callback){
		for(i=0;i<60;i++){
			text = text.replace(eval('/<img data-id="'+(i+1)+'"(.*?)>/g'),'<emt>'+ (i+1) +'</emt>');
		}
		callback(text);
	}
});
