...
//                }
    ...
    var Upload = {
        ...
        config: function (sel) {
            ...
            url=url + (url.indexOf("?") == -1 ? "?" : "&") + "editorid="+me.editor.id + "&token="+token;//初始form提交地址;
        ...
        uploadComplete: function(r){
            console.log(r);
            var me = this;
            //{"error":"请上传一张小于500KB的图片","success":false}
            var data;
            try{
                data =  eval('('+r+')');
            }catch(e){
            ...
            if(data.success){
                ...
                Base.callback(me.editor, me.dialog, json, 'SUCCESS');
            }else{
                Base.callback(me.editor, me.dialog, '', data.error);
            }
        },
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                $('<iframe name="up" id="content-frame" style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
                    var frameObj = document.getElementById("content-frame");
                    var r = $(frameObj.contentWindow.document.body).find('pre').text();
                    if(r == '')return;
                    me.uploadComplete(r);
                    $(this).unbind('load');
                    $(this).remove();
